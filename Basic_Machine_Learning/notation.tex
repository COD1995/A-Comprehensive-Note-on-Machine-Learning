...
\textbf{Regression} is a problem of predicting a real-valued label (often called a \textbf{target}) given an unlabeled example. The regression problem is solved by a \textbf{regression learning algorithm} that taskes a collection of labeled examples as inputs and produces a model that can take an unlabeled example as input and output a target.

\section{Model-Based vs. Instance-Based Learning}
Most supervised learning algorithm are model-based. \textit{Model-based learning algorithms} use the training data to create a \textbf{model} that has \textbf{parameters} learned from the training data. \textit{Instance-based learning algorithms} use the whole dataset as the model. One instance-based algorithm frequently used in practice is \textbf{k-Nearest Neighbors} (kNN).

\section{Shallow vs. Deep Learning}
A \textbf{shallow learning} algorithm learns the parameters of the model directly from the features of the training examples. The \textbf{neural network} learning algorithms, specifically those networks with more than one \textbf{layer} between input and output. Such neural networks are called \textbf{deep neural networks}. In deep neural network learning (or, simply, \textbf{deep learning}), contrary to shallow learning, most model parameters are learned not directly from the features of the training examples, but from the outputs of the preceeding layers.